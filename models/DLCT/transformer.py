...
class Transformer(CaptioningModel):
    ...
    def get_pos_embedding(self, boxes, grids,split=False):
        ...
        if not split:
            ...
            return torch.zeros_like(region_embed),torch.zeros_like(grid_embed)
    ...
    def forward(self, regions, boxes, grids,aligns, seq, *args):
        ...
        pos = torch.cat([region_embed, grid_embed], dim=1)
        dec_output = self.decoder(seq, enc_output, mask_enc, pos=pos,aligns = aligns.unsqueeze(1))
        return dec_output
    ...
    def step(self, t, prev_output, visual, seq, mode='teacher_forcing', **kwargs):
        ...
        if mode == 'teacher_forcing':
            ...
            if t ==0:
                use_aligns = aligns.unsqueeze(1)
            else:
                use_aligns = torch.gather(aligns.unsqueeze(1),1,torch.zeros([int(bs/5),5,50,49],device=aligns.device).long()).view([bs,1,50,49])
            
        return self.decoder(it, self.enc_output, self.mask_enc, pos=self.pos,aligns = use_aligns)
